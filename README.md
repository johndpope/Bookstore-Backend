...
* `docker run -it -p 8080:8080 -v $PWD:/swift-project -w /swift-project myapp-run sh -c .build-ubuntu/release/BookBackend`


#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/BookBackend` directory. You need to make sure you change the `repository` variable in your `chart/BookBackend/values.yaml` file points to the docker image containing your runnable application.
